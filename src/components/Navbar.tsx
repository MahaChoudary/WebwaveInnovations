      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'glass py-2' : 'bg-transparent py-4'}`}>
        <div className="container mx-auto px-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2 group cursor-pointer">
              ...
              <span className="text-xl font-bold font-mono gradient-text"><Webwave Innovations/></span>
            </div>

            {/* Desktop Navigation */}
            <div className="hidden md:flex items-center space-x-8">
              {navItems.map(item => <button key={item.href} onClick={() => scrollToSection(item.href)} className="relative text-muted-foreground hover:text-foreground glow-underline transition-all duration-300 font-medium">
                  {item.label}
                </button>)}
            ...
            {/* Mobile Menu Button */}
            <button className="md:hidden relative z-50 p-2 glass rounded-lg hover:glow-primary transition-all" onClick={() => setIsOpen(!isOpen)}>
        ...
        <div className={`md:hidden fixed inset-0 z-40 transition-all duration-300 ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
          ...
          <div className="relative z-10 flex flex-col items-center justify-center h-full space-y-8">
            {navItems.map((item, index) => <button key={item.href} onClick={() => scrollToSection(item.href)} className={`text-2xl font-semibold glow-underline hover:text-primary transition-all duration-500 ${isOpen ? 'animate-fade-in' : ''}`} style={{
